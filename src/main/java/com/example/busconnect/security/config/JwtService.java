...
public class JwtService {
    ...
    private boolean validateTokenBase(String token, UserDetails userDetails) {
        try {
            ...
            log.debug("Token validation - Username: {}, Matches: {}, Expired: {}, Active: {}", 
                username, usernameMatches, !notExpired, accountActive);
        ...
        try {
            ...
            if (userDetails instanceof CustomUserDetails customUser) {
                if (!customUser.isEnabled()) {
                    log.warn("Account is not active for user: {}", userDetails.getUsername());
                    return false;
                }
            }

            return validateTokenBase(token, userDetails);
    ...
    public boolean isRefreshTokenValid(String token, UserDetails userDetails) {
        try {
            ...
            if (!"refresh".equals(tokenType)) {
                log.warn("Token is not a refresh token, actual type: {}", tokenType);
                return false;
            }
    ...
    public boolean isOfflineTokenValid(String token, UserDetails userDetails) {
        try {
            ...
            if (!"offline".equals(tokenType)) {
                log.warn("Token is not an offline token, actual type: {}", tokenType);
                return false;
            }
