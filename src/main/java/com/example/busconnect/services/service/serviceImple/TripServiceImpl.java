...
import com.example.busconnect.services.service.TripService;
import com.example.busconnect.services.mappers.TripMapper;
...
public class TripServiceImpl  implements TripService {
    ...
    public List<TripResponse> searchTrips(Long routeId, LocalDate date, TripStatus status) {
        ...
        if (status != null) {
            return getTripsByStatus(status); 
        }
...
public void deleteTrip(Long id) {
    ...
    if (trip.getStatus() == TripStatus.ARRIVED) {
        ...
        throw new IllegalStateException("No se puede eliminar un viaje en estado ARRIVED");
    }
    if (trip.getStatus() == TripStatus.DEPARTED) {
        throw new IllegalStateException("No se puede eliminar un viaje en estado DEPARTED");
    }
    ...
    @Override
@Transactional(readOnly = true)
public List<TripResponse> getTodayTrips() {
    LocalDate today = LocalDate.now();
    
    try {
        List<Trip> trips = tripRepository.findTodayTripsWithDetails(today);
        ...
        return trips.stream()
            .map(trip -> {
                try {
                    ...
                    System.err.println("Error al mapear trip " + trip.getId() + ": " + e.getMessage());
                    e.printStackTrace();
                    return null;
                }
            })
        ...
        System.err.println("Error al obtener trips de hoy: " + e.getMessage());
        e.printStackTrace();
        throw new RuntimeException("Error al obtener viajes de hoy", e);
    }
}
